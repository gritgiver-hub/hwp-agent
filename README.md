...
- ✅ **표 셀 편집**: 신청서/양식의 "항목명 | 값" 칸은 **라벨 기준(cell_by_label)** 으로 채움. **표 번호 몰라도 라벨로 표 자동 탐색** + **빈 인접 값칸 자동 선택**(방향 추측 불필요). 주소 기반(row/col)도 goto_addr로 병합 인식. 모든 쓰기는 **적용 후 자동검증** — 실패 시 저장 안 함.
- ✅ **이미지 교체**: **크기+위치(geometry) 보존**하며 교체(인라인/플로팅), 교체 후 파일명으로 **자동검증**.
- ✅ **문서 생성**: 빈 문서에서 제목/문단/표(채움)/이미지로 새 HWP 생성(`executor.generate_document`).
