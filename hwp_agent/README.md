...
- ✅ **표 셀 편집**: 신청서/양식의 "항목명 | 값" 칸은 **라벨 기준(cell_by_label)** 으로 견고하게 채움(병합 값셀 포함). 주소 기반(row/col)도 goto_addr로 병합 인식. 모든 쓰기는 **적용 후 자동검증** — 검증 실패 시 저장 안 함. (병합으로 "덮인" 셀을 직접 주소 지정하면 안전하게 거부)
- ✅ **이미지 교체**: 원래 **크기(geometry)를 보존**하며 교체하고, 교체 후 파일명으로 **자동검증**. 인라인(글자처럼 취급) 이미지는 위치도 정확. (플로팅 이미지의 절대 좌표는 근사 복원)
